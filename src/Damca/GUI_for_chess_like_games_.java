 ...
 */
public class GUI_for_chess_like_games_ extends JFrame {
    ...
    private final PlayableLogic_ gameLogic;
     ...
     * piece's position_ and type. The text color is determined by the piece's owner.
     *
    ...
    private void updateBoard() {
        ...
        for (int row = 0; row < BOARD_SIZE; row++) {
            for (int col = 0; col < BOARD_SIZE; col++) {
                Piece_ piece = gameLogic.getPieceAtPosition(new Position_(row, col));
                if (piece != null) {
                    // Update the button's appearance based on the new piece position_
    ...
    public void start() {
        ...
        for (int col = 0; col < BOARD_SIZE; col++) {
            for (int row = 0; row < BOARD_SIZE; row++) {
                ...
                if ((row + col) % 2 != 0) {
                    buttons[row][col].setBackground(new Color(10, 8, 1));
                }
                else  {
                    buttons[row][col].setBackground(Color.WHITE);
                }
                if (row % (BOARD_SIZE - 1)  == 0 && col % (BOARD_SIZE - 1) == 0) {
                    buttons[row][col].setBackground(new Color(246, 242, 224));
                }

                // Clear default border and content area
                buttons[row][col].setBorderPainted(false);

                if (gameLogic.getPieceAtPosition(new Position_(row, col)) != null) {
                ...
                buttons[row][col].addActionListener((e)-> {
                    ...
                    if (selectedButton == null) {
                        ...
                        Position_ p = new Position_(rowIndexOld, colIndexOld);
                        ...
                        Position_ oldPosition = new Position_(rowIndexOld, colIndexOld);

                        selectedButton = null;

                        int rowIndex = (int) clickedButton.getClientProperty("row");
                        int colIndex = (int) clickedButton.getClientProperty("col");
                        Position_ newPosition = new Position_(rowIndex, colIndex);
